...
public class TodoController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<TodoResponse> updateTodo(@PathVariable("id") Long id, @RequestBody TodoResponse todoResponse) {
        return todoService.getTodo(id)
                .map(todo -> {
                    todoService.updateTodo(id, TodoMapper.toTodoEntity(todoResponse));
                    return todo;
                })
                .map(TodoMapper::toTodoResponse)
                .map(ResponseEntity::ok)
    ...
    @PostMapping("/")
    public ResponseEntity<TodoResponse> createTodo(@RequestBody TodoResponse todoResponse) {
        System.out.println(todoResponse.userId());
        TodoEntity todo = todoService.createTodo(TodoMapper.toTodoEntity(todoResponse));
        return ResponseEntity.ok(TodoMapper.toTodoResponse(todo));
    }
